defmodule T3SystemWeb.Admin.LeagueLive.Index do
  ...
  def render(assigns) do
    ...
    <Layouts.settings flash={@flash} active_item="leagues">
      ...
      >
        <:col :let={{_id, league}} label={gettext("Nome")}>{league.name}</:col>
        <:action :let={{_id, league}}>
          <div class="sr-only">
            <.link navigate={~p"/admin/leagues/#{league}"}>{gettext("Show")}</.link>
          </div>
          <.link navigate={~p"/admin/leagues/#{league}/edit"} class="hover:text-primary-hover">
            {gettext("Edit")}
          </.link>
        </:action>
