defmodule T3SystemWeb.UserAuthTest do
  use T3SystemWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias T3System.Accounts
  alias T3System.Accounts.Scope
  alias T3SystemWeb.UserAuth

  import T3System.AccountsFixtures

  @remember_me_cookie "_t3_system_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, T3SystemWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      T3SystemWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        T3SystemWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
